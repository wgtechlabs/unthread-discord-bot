	...
	describe('isDuplicateMessage', () => {
		describe('Basic Duplicate Detection', () => {
			...
			it('should detect exact duplicate messages', () => {
				const messages = [{ content: 'Hello world' }, { content: 'Another message' }];
		...
		describe('Fuzzy Duplicate Detection', () => {
			...
			it('should detect when existing message is contained in new message', () => {
				...
				expect(isDuplicateMessage(messages, 'This is a short message with extra content')).toBe(
					true,
				);
			});
			...
			it('should only match if length ratio is reasonable', () => {
				const messages = [
					{
						content:
							'Very long message that should not match because the ratio is too different from the short new message content and this would cause false positives in detection',
					},
				];
			...
			it('should log debug information for fuzzy duplicates', () => {
				...
				expect(LogEngine.debug).toHaveBeenCalledWith(
					'Content duplicate message detected (fuzzy match)',
				);
			});
		});
		...
		describe('Edge Cases', () => {
			...
			it('should handle messages with special characters', () => {
				...
				expect(isDuplicateMessage(messages, 'Special chars: !@#$%^&*()_+{}|:"<>?[]\\;\',./')).toBe(
					true,
				);
			});
	...
	describe('processQuotedContent', () => {
		...
		describe('Quoted Content Formatting', () => {
			it('should handle multiple quoted lines', () => {
				...
				const messagesWithMultiline = [{ id: 'msg1', content: 'First message\ncontent here' }];
			...
			it('should stop at first non-quoted line', () => {
				...
				expect(result.replyReference).toBe('msg1');
				expect(result.contentToSend).toBe(
					'Normal line\n> This should not be included\nReply content',
				);
			});
		});
