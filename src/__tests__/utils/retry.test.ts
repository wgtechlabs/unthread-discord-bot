...
describe('withRetry', () => {
  ...
  describe('Configuration Options', () => {
    it('should use default maxAttempts when not specified', async () => {
      ...
      try {
        await withRetry(operation, { baseDelayMs: 10 }); // Use small delay for testing
      } catch (error) {
        // Expected to fail
      }
  ...
  describe('Error Handling', () => {
    ...
    it('should log debug information for each failed attempt', async () => {
      ...
      try {
        await withRetry(operation, { maxAttempts: 2, operationName: 'debug-test', baseDelayMs: 10 });
      } catch (e) {
        // Expected to fail
      }
    ...
    it('should log final error after all attempts', async () => {
      ...
      try {
        await withRetry(operation, { maxAttempts: 2, operationName: 'final-test', baseDelayMs: 10 });
      } catch (e) {
        // Expected to fail
      }
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle very large baseDelayMs', async () => {
      ...
      const result = await withRetry(operation, { 
        maxAttempts: 2, 
        baseDelayMs: 10 // Use reasonable delay for testing
      });
